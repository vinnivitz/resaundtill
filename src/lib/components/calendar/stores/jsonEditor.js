...
const get = () => {
	...
	return {
		...
		updateMappings() {
			...
			const mappings = Array.from(codeChildren)
				.reduce(({ elements = [], basePath = '', lastProperty }, el) => {
					...
					const newLastProperty = el.classList.contains('property') ? removeQuotes(text) : lastProperty;
					...
					if (isPunctuation && text === '{' && lastProperty) newBasePath = `${basePath}${lastProperty}.`;
		...
		json(json) {
			const highlighted = Prism.highlight(JSON.stringify(json, null, 2), Prism.languages.json, 'json');
